...
class Organism(ABC):
    ...
    def getNewXY(self, newXY, range):
        if self._world.isHex():
            ...
            while True:
                ...
                if newXY[0] < self._world.getSizeX() and newXY[0] >= 0 and \
                    newXY[1] < self._world.getSizeY() and newXY[1] >= 0 and \
    ...
    def proliferation(self, attacker):
        if self.__class__ == attacker.__class__ and self._age >= 10 and\
                attacker.getAge() > 10 and random.randint(0, self._age) < math.sqrt(self._age):
            newXY = [0, 0]
            i = 0
            while True:
                self.getNewXY(newXY, 1)
                i = i + 1
                if self._world.getCell(newXY[0], newXY[1]) != None and i < 100:
                    continue
                break
            if i < 100:
